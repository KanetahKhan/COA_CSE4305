...
### CPI Impact Calculator

The simulator includes a simple 5-stage pipeline CPI model:

Effective CPI = Base CPI + (Memory Stalls / Instructions)

- Base CPI defaults to 1.0 and can be changed in the GUI Settings dialog.
- Memory stalls come directly from cache miss behavior (allocate and write-back delays).
- Throughput is reported as IPC (instructions per cycle), so you can see cache performance reduce CPU throughput in real time.
