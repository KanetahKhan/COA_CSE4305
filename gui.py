...
class SimulatorGUI:
    ...
    def _update_stats(self):
        ...
        eff_cpi = f"{stats['effective_cpi']:.3f}" if stats["total_requests"] else "—"
        ipc  = f"{stats['achieved_ipc']:.3f}" if stats["total_requests"] else "—"
    ...
    def _step(self):
        ...
        wb_has_room = len(self._write_buffer) < self._cfg_wb_size
        ...
        if wb_entry is not None and len(self._write_buffer) < self._cfg_wb_size:
            self._write_buffer.append(wb_entry)
            self._max_wb_occupancy = max(self._max_wb_occupancy, len(self._write_buffer))
        ...
        if (curr_state == State.ALLOCATE
            ...
            self.memory.start_write(wb["address"], wb["data"])
            self._mem_op_started = True
        ...
        if completed_write:
            self._mem_wb_block = completed_write_addr & ~(bs - 1)
        ...
        if (self.cpu.is_done() and self.cache_ctrl.state == State.IDLE
            and not self.memory.busy and not self._write_buffer):
    ...
    def _take_snapshot(self):
        ...
        return {
            ...
            "write_buffer":      list(self._write_buffer),
            "max_wb_occupancy":  self._max_wb_occupancy,
        }
    ...
    def _restore_snapshot(self, snap):
        ...
        self._write_buffer       = list(snap.get("write_buffer", []))
        self._max_wb_occupancy   = snap.get("max_wb_occupancy", 0)
